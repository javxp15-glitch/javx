...
export function buildR2ObjectKeyCandidates(url: string): string[] {
  const cleaned = url.split("?")[0]?.split("#")[0]?.trim() ?? ""
  if (!cleaned) return []

  const candidates = new Set<string>()
  const basenameCandidates = new Set<string>()
  ...
  const push = (value: string | null | undefined) => {
    ...
    if (basename) {
      basenameCandidates.add(basename)
    }

    if (normalized.startsWith("source/")) {
      candidates.add(normalized.replace(/^source\//, `${R2_KEY_PREFIX}/`))
    }

    if (normalized.startsWith(`${R2_KEY_PREFIX}/`)) {
      candidates.add(normalized.replace(new RegExp(`^${R2_KEY_PREFIX}/`), "source/"))
      candidates.add(normalized.slice(`${R2_KEY_PREFIX}/`.length))
    }
  }

  push(extractR2Key(cleaned))
  push(cleaned)
    ...
    try {
      ...
      try {
        ...
        if (path.startsWith(bucketPrefix)) {
          push(path.slice(bucketPrefix.length))
        }
      } catch {
        // Ignore config errors while building fallbacks.
      }
    } catch {
  ...
  for (const basename of basenameCandidates) {
    push(basename)
    push(`${R2_KEY_PREFIX}/${basename}`)
    push(`${R2_KEY_PREFIX}/videos/${basename}`)
    push(`source/${basename}`)
  }
...
export async function getSignedPlaybackUrl(url: string | null, expiresIn = 3600): Promise<string | null> {
  if (!url) return url
  const candidates = buildR2ObjectKeyCandidates(url)
  if (!candidates.length) return normalizeR2Url(url)

  for (const key of candidates) {
    try {
      return await getSignedR2Url(key, expiresIn)
    } catch {
      // Try the next candidate when legacy records point at a variant key.
    }
  }
